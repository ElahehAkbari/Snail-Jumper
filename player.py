...
class Player(pygame.sprite.Sprite):
    def __init__(self, game_mode):
        ...
        if self.game_mode == "Neuroevolution":
            ...
            layer_sizes = [8, 16, 1]  # TODO (Design your architecture here by changing the values)
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        # if len(obstacles) > 0:
        #     print("obstacle x", obstacles[0]['x'])
        #     print("obstacle y", obstacles[0]['y'])
        input_vec = self.create_input(screen_width, screen_height, obstacles, player_x, player_y)
        nn_output = self.nn.forward(np.array(input_vec).reshape(len(input_vec), 1))
        if nn_output > 0.5:
    ...
    def create_input(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        #
        # if len(obstacles) > 0:
        #     obstacle1_x = abs(player_x - obstacles[0]['x'] / screen_width) * 2
        #     obstacle1_y = abs((player_y - obstacles[0]['y'] / (screen_height * 3)))
        #
        # if len(obstacles) > 1:
        #     obstacle2_x = abs(player_x - obstacles[1]['x'] / screen_width) * 2
        #     obstacle2_y = abs((player_y - obstacles[1]['y'] / (screen_height * 3)))
        #
        # if len(obstacles) > 2:
        #     if 177 < obstacles[2]['x'] < 410:
        #         obstacle3_x = abs(player_x - obstacles[2]['x'] / screen_width) * 2
        #         obstacle3_y = abs((player_y - obstacles[2]['y'] / (screen_height * 3))) * 1.5
        #     else:
        #         obstacle3_x = abs(player_x - obstacles[2]['x'] / screen_width)
        #         obstacle3_y = abs((player_y - obstacles[2]['y'] / (screen_height * 3)))
        #
        # nn_input = [player_x, player_y, obstacle1_x, obstacle1_y, obstacle2_x, obstacle2_y, obstacle3_x, obstacle3_y]
